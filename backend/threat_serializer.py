...
class ThreatDataSerializer:
    ...
    def deserialize(self, data: bytes, verify_integrity: bool = True) -> ThreatDataPackage:
        """
        ...
        try:
            ...
            if verify_integrity:
                ...
                if computed_checksum != stored_checksum:
                    raise IntegrityError("Checksum verification failed - data may be corrupted")
    ...
    def _indicator_to_dict(self, indicator: ThreatIndicator) -> Dict[str, Any]:
        ...
        return {
            "id": indicator.id,
            "type": indicator.type.value if isinstance(indicator.type, ThreatType) else indicator.type,
            "value": indicator.value,
            "severity": indicator.severity.value if isinstance(indicator.severity, ThreatSeverity) else indicator.severity,
    ...
    def _from_dict(self, data: Dict[str, Any]) -> ThreatDataPackage:
        ...
        return ThreatDataPackage(
            ...
            feeds=feeds,
            total_indicators=data.get("total_indicators", sum(len(f.indicators) for f in feeds)),
    ...
    def _indicator_from_dict(self, data: Dict[str, Any]) -> ThreatIndicator:
        ...
        return ThreatIndicator(
            id=data["id"],
            type=ThreatType(data["type"]) if data["type"] in [t.value for t in ThreatType] else ThreatType.UNKNOWN,
            value=data["value"],
            severity=ThreatSeverity(data["severity"]) if data["severity"] in [s.value for s in ThreatSeverity] else ThreatSeverity.MEDIUM,
            source=data["source"],
