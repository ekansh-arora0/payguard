...
class VideoDeepfakeDetector:
    ...
    def check_video(self, video_path: str, max_frames: int = 30) -> Dict:
        ...
        if not os.path.exists(video_path):
            return {"error": "Video file not found", "is_deepfake": None, "confidence": 0}
        ...
        try:
            ...
            if total_frames == 0:
                return {"error": "Could not read video", "is_deepfake": None, "confidence": 0}
            
            frame_indices = np.linspace(0, total_frames - 1, min(max_frames, total_frames))
                    ...
                    for face in faces:
                        if self.dire_model:
                            ...
                            if score and score > 0.5:
                                ai_face_count += 1
                                self.face_detection_results.append({"frame": int(frame_idx), "score": score, "is_ai": True})
                            else:
                                real_face_count += 1
                                self.face_detection_results.append({"frame": int(frame_idx), "score": score, "is_ai": False})
                        else:
                            real_face_count += 1
