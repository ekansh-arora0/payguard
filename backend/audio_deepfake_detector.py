...
class AudioDeepfakeDetector:
    ...
    def check_audio(self, audio_path: str) -> Dict:
        ...
        try:
            ...
            return {
                "error": "librosa required for audio analysis",
                "is_deepfake": None,
                "confidence": 0,
            }
    ...
    def _add_finding(self, indicator: str, confidence: int, details: str = ""):
        ...
        self.findings.append(
            {"indicator": indicator, "confidence": confidence, "details": details}
        )
    ...
    def _check_spectral_features(self, y, sr):
        ...
        if centroid_mean > 4000:
            self._add_finding(
                "Spectral-Center", 25, f"High centroid: {centroid_mean:.0f}Hz"
            )

        if rolloff_mean > 7000:
            self._add_finding(
                "Spectral-Rolloff", 25, f"High rolloff: {rolloff_mean:.0f}Hz"
            )

        spectral_bandwidth = librosa.feature.spectral_bandwidth(S=S, sr=sr)[0]
        if np.std(spectral_bandwidth) < 500:
            self._add_finding(
                "Spectral-BW", 20, "Unusually uniform bandwidth (AI indicator)"
            )
            ...
            if pitch_values:
                ...
                if pitch_std < 20:
                    self._add_finding(
                        "Pitch-Stable",
                        30,
                        f"Unnaturally stable pitch (std: {pitch_std:.1f})",
                    )
                ...
                if 50 < pitch_mean < 300:
                    ...
                    self._add_finding(
                        "Pitch-Range", 20, f"Unusual pitch: {pitch_mean:.0f}Hz"
                    )
    ...
    def _check_artifacts(self, y, sr):
        ...
        if np.mean(zcr) < 0.05:
            self._add_finding(
                "ZCR-Low", 25, "Very low zero-crossing rate (AI audio often too clean)"
            )
        ...
        if np.std(rms) < 0.02:
            self._add_finding(
                "RMS-Stable", 20, "Unusually consistent amplitude (AI indicator)"
            )

        if np.max(rms) / (np.mean(rms) + 1e-10) < 3:
            self._add_finding(
                "RMS-Dynamic", 25, "Low dynamic range (AI audio characteristic)"
            )
