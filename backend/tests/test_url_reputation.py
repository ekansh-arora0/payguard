...
from url_reputation import (BloomFilter, DomainAgeChecker, OpenPhishFeed,
                            PhishTankFeed, ReputationResult, SSLInfo,
                            SSLInspector, ThreatSource, ThreatType,
                            URLhausFeed, URLReputationService,
                            WhitelistManager, create_url_reputation_service)
...
class TestURLReputationService:
    ...
    def test_batch_check(self, service):
        ...
        urls = ["https://google.com", "https://unknown1.com", "https://unknown2.com"]
...
class TestSSLInspector:
    ...
    def test_domain_matches_cert_exact(self):
        ...
        assert inspector._domain_matches_cert("example.com", "example.com", []) is True
    ...
    def test_domain_matches_cert_wildcard(self):
        ...
        assert (
            inspector._domain_matches_cert("sub.example.com", "*.example.com", [])
            is True
        )

        assert (
            inspector._domain_matches_cert("example.com", "*.example.com", []) is True
        )
    ...
    def test_domain_matches_cert_san(self):
        ...
        assert (
            inspector._domain_matches_cert(
                "api.example.com", "example.com", ["api.example.com", "www.example.com"]
            )
            is True
        )
    ...
    def test_organization_match(self):
        ...
        assert (
            inspector._check_organization_match(
                "google.com", "Google LLC", "google.com"
            )
            is True
        )
        ...
        assert (
            inspector._check_organization_match(
                "example.com", "Unrelated Corp", "example.com"
            )
            is False
        )
...
class TestFactoryFunction:
    ...
    def test_create_service_disabled_features(self):
        ...
        service = create_url_reputation_service(
            enable_domain_age_check=False, enable_ssl_inspection=False
        )
