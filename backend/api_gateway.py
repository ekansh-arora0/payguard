...
class TLSConfig:
    ...
    cipher_suites: List[str] = field(default_factory=lambda: SECURE_CIPHER_SUITES.copy())
...
class AuthFailureLogger:
    ...
    def __init__(self, db=None):
        self.db = db
        self._failure_count: Dict[str, int] = {}  # IP -> count for rate limiting detection
...
class RateLimiter:
    ...
    async def check_rate_limit(
        ...
        minute_requests = len([
            t for t in self._minute_counts.get(identifier, [])
            if t > minute_ago
        ])
        ...
        hour_requests = len([
            t for t in self._hour_counts.get(identifier, [])
            if t > hour_ago
        ])
    ...
    def _cleanup_old_entries(self, identifier: str, now: datetime):
        ...
        if identifier in self._minute_counts:
            self._minute_counts[identifier] = [
                t for t in self._minute_counts[identifier]
                if t > hour_ago
            ]
        
        if identifier in self._hour_counts:
            self._hour_counts[identifier] = [
                t for t in self._hour_counts[identifier]
                if t > hour_ago
            ]
...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, api_key_manager: SecureAPIKeyManager, public_endpoints: set = None):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if not self._is_public_endpoint(path):
            try:
                ...
                api_key_doc = await self.api_key_manager.validate_api_key(api_key, request)
