...
class AdversarialFeedbackDetector:
    ...
    def __init__(self):
        self._session_reports: Dict[str, List[datetime]] = (
            {}
        )  # session_hash -> timestamps
    ...
    def check_feedback(self, feedback: FeedbackEntry) -> tuple[bool, str]:
        ...
        if session in self._session_reports:
            recent = [
                t
                for t in self._session_reports[session]
        ...
        if url in self._url_verdicts:
            ...
            if len(verdicts) >= self.FLIP_FLOP_THRESHOLD:
                ...
                alternating = all(
                    verdicts[i] != verdicts[i + 1] for i in range(len(verdicts) - 1)
                )
                if alternating:
...
class TelemetryService:
    ...
    def _update_aggregated_feedback(self, feedback: FeedbackEntry) -> None:
        ...
        if agg.total_reports >= self.MIN_FEEDBACK_VOLUME:
            agg.confidence = (
                max(agg.safe_reports, agg.dangerous_reports) / agg.total_reports
            )
        else:
            agg.confidence = 0.0
