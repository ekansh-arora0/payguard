    ...
    try:
        ...
        if fp.exists():
            try:
                ...
                pass
        resp = await client.get(url, headers={"User-Agent": "PayGuard/Trainer/1.0"}, follow_redirects=True)
...
class URLHTMLBiLSTM_GHAB_CNN(nn.Module):
    def __init__(self, vocab_size=256, embedding_dim=64, lstm_hidden_dim=128, num_classes=2, url_maxlen=200, html_maxlen=4096, tabular_feature_dim=0):
        super().__init__()
        self.url_maxlen = url_maxlen
        self.html_maxlen = html_maxlen
        ...
        self.url_bilstm = nn.LSTM(embedding_dim, lstm_hidden_dim, bidirectional=True, batch_first=True)
        ...
        if self.tabular_feature_dim > 0:
            self.tabular_fc = nn.Linear(self.tabular_feature_dim, 64) # Simple FC layer for tabular features
    ...
    def forward(self, url_input, html_input, tabular_input=None):
        ...
        if self.tabular_feature_dim > 0 and tabular_input is not None:
            ...
            combined_features = torch.cat((url_features, html_features, tabular_features), dim=1)
        else:
...
def _parse_list(env_var, type_func):
    ...
    if val:
        try:
            ...
            print(f"Warning: Could not parse {env_var} as a list of {type_func.__name__}. Using default.")
    return None
...
async def main():
    ...
    try:
        try:
            ...
            print("Attempting to download additional dataset from Kaggle: taruntiwarihp/phishing-site-urls")
            ...
            new_cands = list(new_base.glob("**/*.csv")) + list(new_base.glob("**/*.txt"))
            for f in new_cands:
                with open(f, "r", encoding="utf-8", errors="ignore") as fh:
                    sniffer = csv.Sniffer()
                    sample = fh.read(4096)
            ...
            if not local:
                print("PG_DATASET_PATH not set and kagglehub unavailable, skipping Kaggle data.")
            else:
                try:
        ...
        print(f"An unexpected error occurred during Kaggle/PG_DATASET_PATH loading: {e}")
    ...
    if von_data and 'von_train_x' in von_data and 'von_train_y' in von_data:
        ...
        print(f"Loaded Von tabular data with {von_tabular_features.shape[0]} samples and {von_tabular_features.shape[1]} features.")
    ...
    if von_tabular_features is not None and von_tabular_labels is not None:
        ...
        print(f"Aligning Von tabular features with Kaggle URL-label pairs. Using {min_len} samples.")
        for i in range(min_len):
            pairs.append((kaggle_pairs[i][0], kaggle_pairs[i][1], von_tabular_features[i]))
        von_tabular_features = von_tabular_features[:min_len] # Truncate von_tabular_features if it was larger
    else:
        pairs = kaggle_pairs
        von_tabular_features = None # Ensure it's None if not used

    print(f"Final prepared {len(pairs)} URL-label pairs (with/without tabular features) for fetching.")

    tabular_feature_dim = von_tabular_features.shape[1] if von_tabular_features is not None else 0

    sem = asyncio.Semaphore(CONCURRENCY)
    prefetch = True # os.environ.get("PG_PREFETCH", "0") == "1"
    results = []
    if prefetch:
        print(f"Starting HTML fetching for {len(pairs)} URLs with concurrency {CONCURRENCY} and timeout {TIMEOUT}s...")
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            async def worker(u, y, tabular_f=None):
                async with sem:
                    h = await _fetch_html(client, u)
                    if h:
                        return (u, y, h, tabular_f) if tabular_f is not None else (u, y, h)
                    return None
    ...
    print(f"Successfully fetched HTML for {len(results)} out of {initial_results_count} URLs.")
    ...
    if tabular_features is not None:
        url_tr, url_te, html_tr, html_te, tabular_tr, tabular_te, y_tr, y_te = train_test_split(
            urls, htmls, tabular_features, labels, test_size=0.2, random_state=42, stratify=labels
        )
        url_tr, url_val, html_tr, html_val, tabular_tr, tabular_val, y_tr, y_val = train_test_split(
            url_tr, html_tr, tabular_tr, y_tr, test_size=0.2, random_state=42, stratify=y_tr
        )
    else:
    ...
    if tabular_features is not None:
        ds_tr_bilstm = URLHTMLDataset(list(zip(url_tr, y_tr, html_tr)), tabular_data=tabular_tr)
        ds_val_bilstm = URLHTMLDataset(list(zip(url_val, y_val, html_val)), tabular_data=tabular_val)
        ds_te_bilstm = URLHTMLDataset(list(zip(url_te, y_te, html_te)), tabular_data=tabular_te)
    else:
    ...
    epochs_list_bilstm = _parse_list("PG_BILSTM_EPOCHS_LIST", int) or [int(os.environ.get("PG_BILSTM_EPOCHS", "7"))]
    dropout_list_bilstm = _parse_list("PG_BILSTM_DROPOUT_LIST", float) or [float(os.environ.get("PG_BILSTM_DROPOUT", "0.2"))]
    lr_list_bilstm = _parse_list("PG_BILSTM_LR_LIST", float) or [float(os.environ.get("PG_BILSTM_LR", "1e-3"))]
    ...
    for (epochs, dropout, lr) in itertools.product(epochs_list_bilstm, dropout_list_bilstm, lr_list_bilstm):
        print(f"\nStarting training for config: Epochs={epochs}, Dropout={dropout}, LR={lr}")
        config_start_time = time.time()
        model_bilstm_current = URLHTMLBiLSTM_GHAB_CNN(tabular_feature_dim=tabular_feature_dim).to(device)
        ...
        for ep in range(epochs):
            ...
            for i, batch_data in enumerate(dl_tr_bilstm):
                ...
                if tabular_feature_dim > 0:
                    ...
                    url_b, html_b, tabular_b, y_b = url_b.to(device), html_b.to(device), tabular_b.to(device), y_b.to(device)
                ...
                print(f"  Elapsed: {elapsed_time:.2f}s, Estimated total remaining: {estimated_remaining_time:.2f}s")
            ...
            with torch.no_grad():
                for batch_data in dl_val_bilstm:
                    if tabular_feature_dim > 0:
                        ...
                        url_v, html_v, tabular_v, y_v = url_v.to(device), html_v.to(device), tabular_v.to(device), y_v.to(device)
                        ov_bilstm = model_bilstm_current(url_v, html_v, tabular_v)
                    else:
                        url_v, html_v, y_v = batch_data
                        url_v, html_v, y_v = url_v.to(device), html_v.to(device), y_v.to(device)
            ...
            print(f"BiLSTM-GHAB-CNN VAL acc ep {ep+1}/{epochs} lr={lr} drop={dropout}: {val_acc_bilstm:.4f}")
            if val_acc_bilstm > best_val_acc_bilstm:
                best_val_acc_bilstm = val_acc_bilstm
                best_cfg_bilstm = (epochs, dropout, lr)
                best_model_bilstm = URLHTMLBiLSTM_GHAB_CNN(tabular_feature_dim=tabular_feature_dim).to(device)
    ...
    if best_model_bilstm is None:
        ...
        best_cfg_bilstm = (epochs_list_bilstm[0], dropout_list_bilstm[0], lr_list_bilstm[0])
    ...
    print(f"BiLSTM-GHAB-CNN best cfg epochs={best_cfg_bilstm[0]} dropout={best_cfg_bilstm[1]} lr={best_cfg_bilstm[2]}")
    ...
    ts_path_bilstm = Path(__file__).parent.parent / "models" / "best_url_html_bilstm_ghab_cnn.pt"
    ts_path_bilstm.parent.mkdir(parents=True, exist_ok=True)
    if tabular_feature_dim > 0:
        traced_bilstm = torch.jit.trace(best_model_bilstm, (torch.randint(0, 256, (1, 200), dtype=torch.long, device=device), torch.randint(0, 256, (1, 4096), dtype=torch.long, device=device), torch.randn(1, tabular_feature_dim, device=device)))
    else:
        traced_bilstm = torch.jit.trace(best_model_bilstm, (torch.randint(0, 256, (1, 200), dtype=torch.long, device=device), torch.randint(0, 256, (1, 4096), dtype=torch.long, device=device)))
