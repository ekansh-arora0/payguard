...
AI_INDICATORS = {
    ...
    "openai": ["openai", "dall-e", "dalle", "chatgpt", "dall·e"],
...
class AIMetadataChecker:
        ...
        for ai_type, keywords in AI_INDICATORS.items():
            for kw in keywords:
                if kw.lower() in exif_lower:
                    self._add_finding(
                        f"EXIF: {ai_type}", 80, f"Found '{kw}' in EXIF data"
                    )
                ...
                for ai_type, keywords in AI_INDICATORS.items():
                    for kw in keywords:
                        if kw.lower() in xmp_str:
                            self._add_finding(
                                f"XMP: {ai_type}", 85, f"Found '{kw}' in XMP data"
                            )
        except Exception:
            pass
                ...
                if chunk_type == "tEXt" or chunk_type == "iTXt":
                    ...
                    for ai_type, keywords in AI_INDICATORS.items():
                        for kw in keywords:
                            if kw.lower() in data_str:
                                self._add_finding(
                                    f"PNG: {ai_type}", 75, f"Found '{kw}' in PNG chunk"
                                )
    ...
    def _check_dimensions_and_aspect(self, img: Image.Image):
        ...
        if width == height and width in [512, 768, 1024, 1024, 1536, 2048]:
            self._add_finding(
                "Dimensions", 15, f"Square {width}x{height} (common AI size)"
            )

        if width % 64 == 0 and height % 64 == 0:
            self._add_finding(
                "Dimensions", 10, "Dimensions are multiples of 64 (common in AI)"
            )
        ...
        try:
            ...
            if high_to_low_ratio > 0.35:
                self._add_finding(
                    "Spectral-HF",
                    35,
                    f"High frequency ratio: {high_to_low_ratio:.3f} (AI indicator)",
                )
            ...
            if freq_entropy < 4.5:
                self._add_finding(
                    "Spectral-ENT",
                    30,
                    f"Low frequency entropy: {freq_entropy:.2f} (unusual pattern)",
                )

            if freq_variance > 1.5:
                self._add_finding(
                    "Spectral-VAR", 25, f"High frequency variance: {freq_variance:.2f}"
                )

            center_region = log_magnitude[
                center_h - 5 : center_h + 5, center_w - 5 : center_w + 5
            ]
            if np.std(center_region) < 0.1:
                self._add_finding(
                    "Spectral-CENTER", 20, "Very smooth center frequency (common in AI)"
                )
